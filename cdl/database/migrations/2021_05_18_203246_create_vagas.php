...
class CreateVagas extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('TBL_VAGAS', function (Blueprint $table) {
            ...
            $table->string('vag_formacao',10);          // possui formação necessaria [ensino fundamental completo]
            $table->char('vag_experiencia',1);          // Possui experiencia ?  [com experiencia ] [sem experiencia]   
            $table->string('vag_tempo',10);             // tempo de experiencia [3 meses ] [6 meses] [1 ano ] ..etc
            ...
            $table->double('vag_salario',8,2);          // salario  da vaga
            $table->char('vag_pcd',1);                  // se possui deficiencia 
            $table->longText('vag_pcd_descricao');      // caso possua deficiencia campo para descrever       
